...
var allSources = Game.rooms["W7N4"].find(FIND_SOURCES);
...
module.exports.loop = function () {
    ...
    var numberOfHarvestersStations = _.sum(Game.creeps, (c) => c.memory.station == 0);
    var numberOfUpgraders = _.sum(Game.creeps, (c) => c.memory.role == 'upgrader');
    var numberOfBuilders = _.sum(Game.creeps, (c) => c.memory.role == 'builder');
    var numberOfRepairers = _.sum(Game.creeps, (c) => c.memory.role == 'repairer');

    var name = undefined;
    var determinedStation = 0;

    // if not enough harvesters
    if (numberOfHarvesters < minimumNumberOfHarvesters) {
        if (numberOfHarvestersStations < 1) {
            determinedStation = 0;
        }
        else {
            determinedStation = 1;
        ...
        name = Game.spawns.Spawn1.createCreep([WORK,WORK,CARRY,MOVE], undefined,
            { role: 'harvester', working: false, station: determinedStation});
    }
